...
const AdminPanel = () => {
  ...
  const handleChange = (e) => {
    ...
    try {
      const res = await axios.post('http://localhost:8080/api/info', formData);

      const token = res.data.token; // Make sure this matches your backend response
      const fullUrl = `http://localhost:8080/user/public/view/${token}`;
      setQrUrl(fullUrl);

      // Navigate with QR link or token if needed
      navigate('/success', { state: { formData, qrUrl: fullUrl } });
